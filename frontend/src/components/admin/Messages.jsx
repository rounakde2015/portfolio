...
export const Messages = ({ onLogout }) => {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      // Notification for new unread messages (skip on first load)
              ...
              {["all", "unread", "read"].map((f) => (
                ...
                >
                  ...
                  {f === "unread" && stats.unread > 0 && (
                    <span className="ml-1.5 text-[#00E5FF]">
                      {stats.unread}
                    </span>
                  )}
                </button>
          ...
          {!selected ? (
            ...
            <div>
              ...
              <div className="mt-6 flex items-center gap-3 font-mono text-[10px] uppercase tracking-widest text-white/40">
                ...
                ></span>
                Email notification{" "}
                {selected.email_sent ? "delivered" : "not sent"}
              </div>
            </div>
          )}
        </section>
